...
export default function AdminDocManager() {
    ...
    // UI-only filtering (+ soft delete visibility)
    const filtered = useMemo(() => {
        const t = search.trim().toLowerCase();
        return rows.filter((r) => {
            if (!showDeleted && r.isDeleted === true) return false;
            if (showDeleted && r.isDeleted !== true) return false;
    ...
    }, [rows, search, onlyMandatory, showDeleted]);
        ...
        >
            <Paper elevation={12} sx={{ bgcolor: "#fff", borderRadius: 0, width: "100%", px: 0, py: 0 }}>
                <Box sx={{ maxWidth: 1200, mx: "auto", px: { xs: 2, md: 5 }, py: { xs: 2, md: 4 } }}>
                    <TopBar onBack={() => navigate("/admin-dashboard")} onCreate={openCreate} />
