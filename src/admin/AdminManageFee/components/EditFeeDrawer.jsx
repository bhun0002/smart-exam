...
export default function EditFeeDrawer({ open, onClose, onSubmit, courses = [], intakes = [], initial }) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setIntakeId(initial?.intakeId || "");
      setAmount(String(initial?.amount ?? ""));
      setCurrency((initial?.currency || "CAD").toUpperCase());
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: { xs: "100%", sm: 500 } } }}>
      <Box sx={{ p: 3, display: "flex", flexDirection: "column", height: "100%" }}>
        <Typography variant="h6" sx={{ fontWeight: 700, mb: 1 }}>Edit Fee</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Update the Course, Intake, amount, currency, or notes.
        </Typography>
        <Divider sx={{ mb: 2 }} />
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ display: "grid", gap: 2 }}>
          <FormControl>
            <InputLabel id="edit-fee-course">Course</InputLabel>
            <Select
              labelId="edit-fee-course" label="Course"
          ...
          <FormControl>
            <InputLabel id="edit-fee-intake">Intake</InputLabel>
            <Select
              labelId="edit-fee-intake" label="Intake"
