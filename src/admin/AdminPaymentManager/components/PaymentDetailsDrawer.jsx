...
import {
  Drawer, Box, Typography, Divider, Stack, IconButton, Tooltip, Chip,
} from "@mui/material";
...
/* ---- same verified helpers as the table ---- */
...
const Row = ({ label, value, copyable }) => (
  <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1 }}>
    ...
    {copyable && value ? (
      <Tooltip title="Copy">
        <IconButton size="small" onClick={() => copy(value)}><CopyIcon fontSize="inherit" /></IconButton>
      </Tooltip>
    ) : null}
  </Stack>
);

export default function PaymentDetailsDrawer({ open, onClose, payment }) {
  const pretty = useMemo(() => { try { return JSON.stringify(payment, null, 2); } catch { return ""; } }, [payment]);
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: { xs: "100%", sm: 560 }, borderLeft: "1px solid #eee" } }}>
      <Box sx={{ p: 3, display: "flex", flexDirection: "column", height: "100%", gap: 2, bgcolor: "#fff" }}>
        <Typography variant="h6" sx={{ color: "#1A237E", fontWeight: "bold" }}>Payment Details</Typography>
        <Divider />

        <Row label="Amount" value={typeof payment?.amount?.value === "number" ? `${payment.amount.value.toFixed(2)} ${payment?.amount?.currency || "CAD"}` : "-"} />
        ...
        <Box component="pre" sx={{ fontFamily: "monospace", fontSize: 12, bgcolor: "#fafafa", p: 1.5, borderRadius: 1, maxHeight: 320, overflow: "auto", border: "1px solid #eee" }}>
          {pretty}
        </Box>
      </Box>
    </Drawer>
